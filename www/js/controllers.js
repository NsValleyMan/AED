    ...
    return $http.get("http://213.168.249.135:4000/api/getBuildings").then(function(response) {
        ...
        for (i = 0; i < $scope.buildings.length; i++) { 
          ...
          if (lat != "" && lng != ""){
            ...
            var content = '<div><b>' + $scope.buildings[i].name + '</b>  <small>(' + $scope.buildings[i].code + ')</small> <input type="checkbox" ng-click="select(buildings[' + i + '])"></input><br>' + 
